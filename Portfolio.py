...
class Portfolio():
    def __init__(self,connector=None, db_path=None, db_name=None):
        self.portfolio = {}
        ...
        self.update_build_flag = self.__table_exists(self.portfolio_table, [])
        return
    ...
    def build(self, trade_table, freshtrades: TradeBook, connector=None, db_path=None, db_name=None):
        ...
        :param freshtrades: The tradebook object
        :return: None
        '''
        update_build_flag = 0  # build =0, update =1
        if self.update_build_flag == 0:  # Build = 0
            ...
            res = self.cursor.execute('''select max(Date) from ''' + self.portfolio_table).fetchall()  # query for the last pf date

            res_pf_table = self.cursor.execute('''SELECT Ticker,Qty,Price from ''' + self.portfolio_table + ''' where Date =:date_time''',
        ...
        if len(self.portfolio.keys())>0:
            self.update_from = min(self.portfolio.keys())
        else:
            self.update_from = datetime.now().strftime("%Y-%m-%d 00:00:00")
        return
    ...
    def write_db(self, connection=None, db_path=None, db_name=None):
        try:
            ...
            if self.update_build_flag:
                self.cursor.execute('''DELETE FROM PORTFOLIO WHERE Date>=:date_time''', {'date_time': k})

            for k in self.portfolio.keys():
                for pos in self.portfolio[k].keys():
                    self.cursor.execute('''INSERT INTO Portfolio ('Date','Ticker','Price','Qty') VALUES (?,?,?,?)''',(k, pos, self.portfolio[k][pos][1], self.portfolio[k][pos][0]))
